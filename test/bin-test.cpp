...
class BinTest : public TntdbTestBase
{
public:
    BinTest()
      : TntdbTestBase("bin")
    {
        registerMethod("testInOUt", *this, &BinTest::testInOut);
    }

    void testInOut()
    {
        tntdb::Statement ins = conn.prepare(R"SQL(
          insert into tntdbtest(blobcol)
           values(:blobcol)
          )SQL");

        tntdb::Statement sel = conn.prepare(R"SQL(
          select blobcol from tntdbtest
          )SQL");

        TestObject in(42, "Hi there");
        TestObject out;

        ins.set("blobcol", cxxtools::bin::Bin(in))
           .execute();

        sel.selectValue()
           .get(cxxtools::bin::Bin(out));
